...
describe('E2E: SDK', async () => {
  ...
  describe('Create', async () => {
    describe('Initalization', () => {
      ...
      it('should return ens config', async () => {
        const expectedEnsAddress = relayer.getConfig().networkConfig[network].chainSpec.ensAddress;
        const response = await sdk.getRelayerConfig();
        expect(response!.networkConfig[network].chainSpec.ensAddress).to.eq(expectedEnsAddress);
      });
    });
  });
  ...
  describe('Execute signed message', async () => {
    ...
    it('Throws when the gas limit is above the relayers maxGasLimit', async () => {
      ...
      secondSdk.sdkConfig.paymentOptions.gasLimit = (await secondSdk.getRelayerConfig()).networkConfig[network].maxGasLimit + 1;
