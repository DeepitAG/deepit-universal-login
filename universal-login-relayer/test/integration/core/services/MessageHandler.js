...
describe('INT: MessageHandler', async () => {
  ...
  beforeEach(async () => {
    ({multiChainService, wallet, provider, messageHandler, authorisationStore, walletContract, otherWallet, devicesStore, executionWorker} = await setupMessageService(knex, config));
  ...
  it('Error when not enough gas', async () => {
    ...
    await expect(messageHandler.handleMessage(signedMessage, network)).to.be.rejectedWith(`Insufficient Gas. Got GasLimitExecution 1 but should greater than ${GAS_BASE}`);
  });
